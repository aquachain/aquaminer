...
// cpu mask
#if (__GNUC__ > 4) || ((__GNUC__ == 4) && (__GNUC_MINOR__ >= 8))
    if ((mask & ((unsigned __int128)1ULL << i)))
      CPU_SET(i, &set);
#else
    if ((ncpus > 64) || (mask & (1ULL << i)))
      CPU_SET(i, &set);
#endif
  }
  if (id == -1) {
      ...
      if (!work->height) {
        ...
        if (work->height > g_work.height) {
          ...
          if (!opt_quiet) {
            ...
            applog(LOG_BLUE, "%s block %d, %s", algo_names[opt_algo],
                   work->height, netinfo);
          }
        }
      }
...
static int share_result(int result, struct work *work, const char *reason) {
  ...
  if (sharediff == 0) {
    ...
    applog(LOG_NOTICE, "%s %lu/%lu (%s%%), %s %sH, %s %sH/s", sres,
           (result ? accepted_count : rejected_count), total_submits, rate_s,
           hc, hc_units, hr, hr_units);
#else
    applog(LOG_NOTICE, "%s %lu/%lu (%s%%), %s %sH, %s %sH/s, %dC", sres,
           (result ? accepted_count : rejected_count), total_submits, rate_s,
           hc, hc_units, hr, hr_units, (uint32_t)cpu_temp(0));
#endif
  } else {
#if ((defined(_WIN64) || defined(__WINDOWS__)))
    applog(LOG_NOTICE, "%s %lu/%lu (%s%%), diff %.3g%s, %s %sH/s", sres,
           (result ? accepted_count : rejected_count), total_submits, rate_s,
           sharediff, sol, hr, hr_units);
#else
    applog(LOG_NOTICE, "%s %lu/%lu (%s%%), diff %.3g%s, %s %sH/s, %dC", sres,
           (result ? accepted_count : rejected_count), total_submits, rate_s,
           sharediff, sol, hr, hr_units, (uint32_t)cpu_temp(0));
#endif
  }
...
void std_le_build_stratum_request(char *req, struct work *work) {
  ...
  snprintf(req, JSON_BUF_LEN, "{\"method\": \"mining.submit\", \"params\": "
                              "[\"%s\", \"%s\", \"%s\", \"%s\", \"%s\"], "
                              "\"id\":4}",
...
void jr2_build_stratum_request(char *req, struct work *work) {
  ...
  snprintf(req, JSON_BUF_LEN, "{\"method\": \"submit\", \"params\": {\"id\": "
                              "\"%s\", \"job_id\": \"%s\", \"nonce\": \"%s\", "
                              "\"result\": \"%s\"}, \"id\":4}",
...
char *std_malloc_txs_request(struct work *work) {
  ...
  if (work->workid) {
    ...
    sprintf(req, "{\"method\": \"submitblock\", \"params\": [\"%s%s\", %s], "
                 "\"id\":4}\r\n",
...
static const char *gbt_req =
    ...
    "[{\"capabilities\": " GBT_CAPABILITIES "}], \"id\":0}\r\n";
const char *gbt_lp_req = "{\"method\": \"getblocktemplate\", \"params\": "
...
void jr2_get_new_work(struct work *work, struct work *g_work, int thr_id,
  ...
  if (memcmp(work->data, g_work->data, algo_gate.nonce_index) ||
    ...
    work_copy(work, g_work);
    *nonceptr =
        (0xffffffU / opt_n_threads) * thr_id + (*nonceptr & 0xff000000U);
...
static void *miner_thread(void *userdata) {
  ...
  uint32_t end_nonce =
      opt_benchmark ? (0xffffffffU / opt_n_threads) * (thr_id + 1) - 0x20 : 0;
  ...
  if (!opt_benchmark && opt_priority == 0) {
    ...
    if (opt_debug)
      applog(LOG_DEBUG, "Thread %d priority %d (nice %d)", thr_id, opt_priority,
             prio);
#endif
  ...
  */
...
#if (__GNUC__ > 4) || ((__GNUC__ == 4) && (__GNUC_MINOR__ >= 8))
      affine_to_cpu_mask(thr_id, (unsigned __int128)1LL << (thr_id % num_cpus));
#else
  ...
  while (1) {
    ...
    // Scan for nonce
    nonce_found = algo_gate.scanhash(thr_id, &work, max_nonce, &hashes_done);
    ...
    if (nonce_found && !opt_benchmark) { // 4 way with multiple nonces, copy
                                         // individually to work and submit.
      if (nonce_found > 1)
    ...
    if (!opt_quiet) {
      ...
      if (hashcount) {
        ...
        if (hc_units[0])
          ...
          sprintf(hc, "%.0f", hashcount);
        sprintf(hr, "%.2f", hashrate);
        applog(LOG_INFO, "CPU #%d: %s %sH, %s %sH/s", thr_id, hc, hc_units, hr,
               hr_units);
      }
    }
    ...
    if (opt_benchmark && thr_id == opt_n_threads - 1) {
      ...
      if (hashcount) {
        ...
        sprintf(hr, "%.2f", hashrate);
#if ((defined(_WIN64) || defined(__WINDOWS__)))
        applog(LOG_NOTICE, "Total: %s %sH, %s %sH/s", hc, hc_units, hr,
               hr_units);
#else
        applog(LOG_NOTICE, "Total: %s %sH, %s %sH/s, %dC", hc, hc_units, hr,
               hr_units, (uint32_t)cpu_temp(0));
#endif
      }
    }
  } // miner_thread loop
...
json_t *jr2_longpoll_rpc_call(CURL *curl, int *err) {
  ...
  pthread_mutex_unlock(&rpc2_login_lock);
  val =
      json_rpc2_call(curl, rpc_url, rpc_userpass, req, err, JSON_RPC_LONGPOLL);
  return val;
}
...
bool check_cpu_capability() {
  ...
  use_none =
      !(use_sse2 || use_aes || use_sse42 || use_avx512 || use_avx2 || use_sha);

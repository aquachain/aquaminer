...
double target_to_diff(uint32_t *target) {
  ...
  uint64_t m = (uint64_t)tgt[29] << 56 | (uint64_t)tgt[28] << 48 |
               (uint64_t)tgt[27] << 40 | (uint64_t)tgt[26] << 32 |
               (uint64_t)tgt[25] << 24 | (uint64_t)tgt[24] << 16 |
               (uint64_t)tgt[23] << 8 | (uint64_t)tgt[22] << 0;
...
static bool stratum_parse_extranonce(struct stratum_ctx *sctx, json_t *params,
  ...
  if (pndx == 0 && opt_debug) /* pool dynamic change */
    applog(LOG_DEBUG, "Stratum set nonce %s with extranonce2 size=%d", xnonce1,
           xn2_size);
...
bool stratum_authorize(struct stratum_ctx *sctx, const char *user,
  ...
  if (jsonrpc_2) {
    ...
    sprintf(s, "{\"id\": 2, \"method\": \"mining.authorize\", \"params\": "
               "[\"%s\", \"%s\"]}",
            user, pass);
  }
  ...
  if (sret) {
    ...
    if (!extra) {
      ...
      //json_is_false(res_val)))
      //				applog(LOG_DEBUG, "extranonce subscribe
      //not supported");
      json_decref(extra);
    }
    free(sret);
...
static bool stratum_notify(struct stratum_ctx *sctx, json_t *params) {
  ...
  if (has_claim)
    hex2bin(sctx->job.claim, claim, 32);
